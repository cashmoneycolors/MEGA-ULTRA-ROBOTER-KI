...
def _make_payload(amount: float, currency: str, event_type: str) -> dict:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Send local test PayPal webhooks to webhook_server.py"
    )
    parser.add_argument(
        "--url",
        ...
        help="Webhook endpoint URL (default: http://127.0.0.1:8503/paypal/webhook)",
    )
    parser.add_argument(
        "--count", type=int, default=int(os.getenv("TEST_WEBHOOK_COUNT", "1"))
    )
    parser.add_argument(
        "--amount", type=float, default=float(os.getenv("TEST_WEBHOOK_AMOUNT", "10"))
    )
    parser.add_argument("--currency", default=os.getenv("TEST_WEBHOOK_CCY", "EUR"))
    parser.add_argument(
        "--event-type",
        default=os.getenv("TEST_WEBHOOK_EVENT_TYPE", "PAYMENT.CAPTURE.COMPLETED"),
    )
    parser.add_argument(
        "--print-every",
        type=int,
    ...
    for i in range(args.count):
        ...
        try:
            ...
            if (i + 1) % print_every == 0 or (i + 1) == args.count:
                ...
                rps = (i + 1) / elapsed
                print(
                    f"[{i+1}/{args.count}] ok={ok} errors={errors} last_http={resp.status_code} rps={rps:.1f}"
                )
            ...
            if (i + 1) % print_every == 0 or (i + 1) == args.count:
                ...
                rps = (i + 1) / elapsed
                print(
                    f"[{i+1}/{args.count}] ok={ok} errors={errors} last_error={e} rps={rps:.1f}"
                )
    ...
    print(
        f"done: sent={args.count} ok={ok} errors={errors} url={args.url} seconds={total_s:.2f}"
    )
