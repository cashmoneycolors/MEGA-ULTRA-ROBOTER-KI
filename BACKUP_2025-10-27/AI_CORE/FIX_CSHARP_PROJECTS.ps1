...
#
# WICHTIG: Secrets wie JWT_SECRET und MAINTENANCE_KEY dürfen NIEMALS im Skript oder Quellcode hardcodiert werden!
# Prüfe, ob die Umgebungsvariablen gesetzt sind, sonst Warnung anzeigen.
if (-not $env:JWT_SECRET -or -not $env:MAINTENANCE_KEY) {
    Write-Host "[WARNUNG] JWT_SECRET oder MAINTENANCE_KEY ist NICHT gesetzt! Bitte als Umgebungsvariable setzen (z.B. $env:JWT_SECRET=...) – Niemals im Skript speichern!" -ForegroundColor Yellow
}
