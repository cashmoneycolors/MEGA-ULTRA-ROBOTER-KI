...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.last_approve_url:
            st.link_button("🔗 Zur PayPal Zahlung", st.session_state.last_approve_url)
        ...
        if st.button("✅ Capture (nach Approval)"):
            ...
            if not oid:
                ...
                try:
                    resp = requests.post(
                        capture_url, json={"order_id": oid}, timeout=12
                    )
                    if resp.status_code == 200:
                        data = resp.json() or {}
                        st.session_state.logs.append(
    ...
    # Control Panel
    st.markdown("### 🛠️ OPERATION CENTER")
    ...
    with c1:
        ...
        if st.button("🧪 TEST (Requires REAL webhook)"):
            st.toast("Kein Demo-Sale. Sende echte PayPal Zahlung / Webhook.")
            st.session_state.logs.append(
