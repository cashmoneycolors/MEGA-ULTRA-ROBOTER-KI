...
class WealthSystemFinal:
    def __init__(self, root):
        self.root = root
        self.root.title("Wealth System - PayPal Live")
    ...
    def load_config(self):
        config_path = Path("paypal_config.json")
        if config_path.exists():
            try:
                with open(config_path) as f:
                    config = json.load(f)
                    self.paypal_client_id = config.get("paypal", {}).get("client_id", "")
                    self.paypal_secret = config.get("paypal", {}).get("client_secret", "")
                    self.paypal_email = config.get("paypal", {}).get("email", "")
            except:
                pass
    ...
    def setup_ui(self):
        ...
        title = tk.Label(header, text="WEALTH SYSTEM - PayPal Live Edition", 
        ...
        self.client_id_entry = tk.Entry(paypal_frame, width=25, bg="#0a0a0a", fg="#00ff00")
        self.client_id_entry.insert(0, self.paypal_client_id)
        self.client_id_entry.pack(side=tk.LEFT, padx=5)
        
        tk.Label(paypal_frame, text="Secret:", bg="#1a1a1a", fg="#888").pack(side=tk.LEFT, padx=5)
        self.secret_entry = tk.Entry(paypal_frame, width=25, bg="#0a0a0a", fg="#00ff00", show="*")
        self.secret_entry.insert(0, self.paypal_secret)
        self.secret_entry.pack(side=tk.LEFT, padx=5)
        
        tk.Label(paypal_frame, text="Email:", bg="#1a1a1a", fg="#888").pack(side=tk.LEFT, padx=5)
        self.email_entry = tk.Entry(paypal_frame, width=20, bg="#0a0a0a", fg="#00ff00")
        self.email_entry.insert(0, self.paypal_email)
        self.email_entry.pack(side=tk.LEFT, padx=5)
        
        tk.Button(paypal_frame, text="Save", command=self.save_config,
        ...
        self.stats_labels = {}
        stats = [("Capital:", "capital"), ("Profit:", "profit"), ("Cycles:", "cycles"), ("Status:", "status")]
        ...
        for i, (label, key) in enumerate(stats):
            ...
            val = tk.Label(stats_frame, text="0.00", bg="#1a1a1a", fg="#00ff00", font=("Arial", 10, "bold"))
        ...
        buttons = [("Load Funds", self.load_funds), ("Start System", self.start_system), 
                  ("Withdraw", self.withdraw_profit), ("Refresh", self.update_stats)]
        
        for text, cmd in buttons:
            btn = tk.Button(btn_frame, text=text, command=cmd, 
                           bg="#00ff00", fg="#000", font=("Arial", 9, "bold"), padx=10, pady=5)
    ...
    def save_config(self):
        ...
        config = {"paypal": {"client_id": client_id, "client_secret": secret, "email": email}}
        ...
        self.log("PayPal config saved")
    
    def load_funds(self):
        amount = simpledialog.askfloat("Load Funds", "Amount (CHF):")
        if amount and amount > 0:
            self.log(f"Loading {amount:.2f} CHF...")
    ...
    def _load_funds_thread(self, amount):
        try:
            ...
            conn.commit()
            conn.close()
            self.log(f"[OK] Loaded {amount:.2f} CHF")
        except Exception as e:
            self.log(f"[ERROR] {e}")
    
    def withdraw_profit(self):
        amount = simpledialog.askfloat("Withdraw", "Amount (CHF):")
        if amount and amount > 0:
            self.log(f"Withdrawing {amount:.2f} CHF...")
    ...
    def _withdraw_thread(self, amount):
        try:
            ...
            payout = paypalrestsdk.Payout({
                "sender_batch_header": {
                    "sender_batch_id": f"batch_{int(amount*100)}",
                    "email_subject": "Wealth System"
                },
                "items": [{
            ...
            if payout.create():
                ...
                conn.commit()
                conn.close()
                self.log(f"[OK] Withdrawn {amount:.2f} CHF")
            else:
                self.log(f"[ERROR] {payout.error}")
        except Exception as e:
            self.log(f"[ERROR] {e}")
    ...
    def get_stats(self):
        try:
            ...
            cycles = c.fetchone()[0]
            conn.close()
            return {"capital": capital, "profit": profit, "cycles": cycles, "status": "RUNNING" if self.system_running else "IDLE"}
        except:
            return None
